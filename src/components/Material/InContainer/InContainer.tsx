...
const MaterialDeleteDialog = React.lazy(() =>
  import('../../Dialogs/DeleteDialog/MaterialDeleteDialog')
);
...
class InContainer extends React.Component<Props, State> {
  ...
  removeMaterialItem = async () => {
    ...
    if (toRemoveMaterial._id) {
      try {
        ...
        this.setState({
          deleteDialogOpen: false,
          materialsList: this.state.materialsList.filter(
            (item) => item._id !== toRemoveMaterial._id
          ),
        });
        this.props.setLoading(false);
      } catch (err) {
  ...
  render() {
    ...
    const remindWord = toRemoveMaterial
      ? `确定删除${toRemoveMaterial.name}吗？`
      : '确定删除吗？';
    return (
      <>
        <div className={styles.inctn}>
          ...
          {tabValueSecond === 0 && renderListItems}
