...
const ApplyWorkDialog = React.lazy(() =>
  import('../../../components/Dialogs/ConfirmDialog/ApplyWorkDialog')
);
const BasicDeleteDialog = React.lazy(() =>
  import('../../../components/Dialogs/DeleteDialog/BasicDeleteDialog')
);
const CreateWorkDialog = React.lazy(() =>
  import('../../../components/Dialogs/ConfirmDialog/CreateWorkDialog')
);
...
class Works extends React.Component<Props, State> {
  ...
  handleWorkDelete = async () => {
    try {
      ...
      if (item && item._id) {
        ...
        const workListFilter = this.state.workList.filter(
  ...
  render() {
    ...
    const remindWord = `确定删除${
      choosenWork ? choosenWork.settings.title : ''
    }?`;
    const applyWorkRemindWord = `确定放弃现有工作区，进行编辑${
      choosenWork ? choosenWork.settings.title : ''
    }?`;
    const renderWorkCards = workList.map((item: Work, index) => {
      return (
        <WorkCard
          ...
          handleWorkChooseToDelete={(item) =>
            this.handleChooseWorkToDelete(item)
          }
        />
      );
    });
    return (
      <div className={styles.works}>
        <div className={styles.workflex}>{renderWorkCards}</div>
