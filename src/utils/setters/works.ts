...
export const addComOrderInCurrentPage = (
  state: State,
...
export const removeComOrderInCurrentPage = (
  state: State,
  targetPageId: number,
  comId: number
) => {
...
export const exchangeOrderInPage = (
  state: State,
  targetPageId: number,
  oldComId: number,
  newComId: number
  ...
  stateCopy.map((item: Page, pageIndex) => {
    if (item.id === targetPageId) {
      const oldIndex = stateCopy[pageIndex].order.findIndex(
        (e) => e === oldComId
      );
      const newIndex = stateCopy[pageIndex].order.findIndex(
        (e) => e === newComId
      );
      afterMove[pageIndex].order = arrayMove(
        stateCopy[pageIndex].order,
        oldIndex,
        newIndex
      );
    }
...
export const setPageSettingsByPageId = (
  state: State,
...
export const setPageStylesByPageId = (
  state: State,
