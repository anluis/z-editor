  ...
  render() {
    ...
    if (findPageResult.hasOwnProperty('settings')) {
      if (findPageResult.settings.hasOwnProperty('pageTitle')) {
        document.title = findPageResult.settings.pageTitle
          ? findPageResult.settings.pageTitle
          : '星视度';
      }
    }
    ...
    const bindPageStyles = {
      minHeight:
        findPageResult.styles.height *
          (window.innerWidth / findPageResult.styles.width) +
        'px',
    };
    return (
      <div className={styles.workroot} style={bindPageStyles}>
        {RenderComs}
      </div>
    );
  }
